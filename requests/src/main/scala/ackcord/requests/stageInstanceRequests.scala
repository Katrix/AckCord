...
case class CreateStageInstance(
    params: CreateStageInstanceData,
  ...
  override def withReason(reason: String): CreateStageInstance =
    copy(reason = Some(reason))
}

case class GetStageInstance(channelId: StageGuildChannelId)
    extends NoParamsNiceResponseRequest[StageInstance] {
...
object UpdateStageInstanceData {
  implicit val encoder: Encoder[UpdateStageInstanceData] =
    (a: UpdateStageInstanceData) =>
...
case class UpdateStageInstance(
  ...
  override def paramsEncoder: Encoder[UpdateStageInstanceData] =
    UpdateStageInstanceData.encoder
  override def responseDecoder: Decoder[StageInstance] = Decoder[StageInstance]

  override def withReason(reason: String): UpdateStageInstance =
    copy(reason = Some(reason))
}

case class DeleteStageInstance(
    channelId: StageGuildChannelId,
    reason: Option[String] = None
  ...
  override def withReason(reason: String): DeleteStageInstance =
    copy(reason = Some(reason))
}
