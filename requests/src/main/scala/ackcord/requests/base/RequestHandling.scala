...
object RequestHandling {
  ...
  private def timeTilReset[A](relativeTime: Boolean, response: SttpResponse[A]): FiniteDuration = {
    if (relativeTime) {
      response.headers.find(_.name == "X-RateLimit-Reset-After").fold(-1.millis)(_.value.toDouble.seconds)
    } else {
      response.headers.find(_.name == "X-RateLimit-Reset").fold(-1.millis) { header =>
        ...
        (Instant.ofEpochSecond(header.value.toLong).toEpochMilli - System.currentTimeMillis()).millis.max(1.milli)
      }
    }
  }
  ...
  def runRequestWithoutRatelimits[Response, R, R1 >: R, F[_]](
    ...
    backend.responseMonad.handleError[RequestAnswer[Response]] {
      for {
        ...
        _ <- monad.blocking(logResponse(resWithStrBody.copy(resWithStrBody.body._2)))
        res = resWithStrBody.copy(body = resWithStrBody.body._1)
        ratelimitInfo = RatelimitInfo(
          timeTilReset(settings.relativeTime, res),
        ...
        case (_, StatusCode.TooManyRequests) =>
          FailedRequest.RequestRatelimited(isGlobalRatelimit(res), ratelimitInfo, request.route, request.identifier)
  ...
  def runRequest[Response, R, F[_]](
    ...
    settings.ratelimiter
      .ratelimitRequest(request.route, request)
