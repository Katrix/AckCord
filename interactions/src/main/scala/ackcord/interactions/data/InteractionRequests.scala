...
object InteractionRequests {
  ...
  def createInteractionResponse(
    ...
    Request.restRequest(
      route = (Route.Empty / "interactions" / Parameters[InteractionId]("interactionId", interactionId) / Parameters[String]("interactionToken", interactionToken) / "callback").toRequest(Method.POST),
      params = body,
  ...
  def getOriginalInteractionResponse(applicationId: ApplicationId, interactionToken: String): Request[Unit, Message] = 
    Request.restRequest(
      route = (Route.Empty / "webhooks" / Parameters[ApplicationId]("applicationId", applicationId) / Parameters[String]("interactionToken", interactionToken) / "messages" / "@original").toRequest(Method.GET),
  ...
  class EditOriginalInteractionResponseBody(json: Json, cache: Map[String, Any] = Map.empty) extends DiscordObject(json, cache) {
    ...
    override def values: Seq[() => Any] = Seq(() => content, () => embeds, () => allowedMentions, () => components, () => attachments)
  }
  object EditOriginalInteractionResponseBody extends DiscordObjectCompanion[EditOriginalInteractionResponseBody] with CreateMessageLikeMixin[EditOriginalInteractionResponseBody] {
    ...
    ): EditOriginalInteractionResponseBody = makeRawFromFields("content" :=? content, "embeds" :=? embeds, "allowed_mentions" :=? allowedMentions, "components" :=? components, "attachments" :=? attachments)
  }
  ...
  def editOriginalInteractionResponse[MPR](
    ...
    Request.complexRestRequest(
      route = (Route.Empty / "webhooks" / Parameters[ApplicationId]("applicationId", applicationId) / Parameters[String]("interactionToken", interactionToken) / "messages" / "@original").toRequest(Method.PATCH),
      params = body,
      
      requestBody = Some(EncodeBody.MultipartBody(
  ...
  def deleteOriginalInteractionResponse(applicationId: ApplicationId, interactionToken: String): Request[Unit, Unit] = 
    Request.restRequest(
      route = (Route.Empty / "webhooks" / Parameters[ApplicationId]("applicationId", applicationId) / Parameters[String]("interactionToken", interactionToken) / "messages" / "@original").toRequest(Method.DELETE),
  ...
  class CreateFollowupMessageBody(json: Json, cache: Map[String, Any] = Map.empty) extends DiscordObject(json, cache) {
    ...
    override def values: Seq[() => Any] = Seq(() => content, () => tts, () => embeds, () => allowedMentions, () => components, () => attachments, () => flags, () => threadName)
  }
  object CreateFollowupMessageBody extends DiscordObjectCompanion[CreateFollowupMessageBody] with CreateMessageLikeMixin[CreateFollowupMessageBody] {
    ...
    ): CreateFollowupMessageBody = makeRawFromFields("content" :=? content, "tts" :=? tts, "embeds" :=? embeds, "allowed_mentions" :=? allowedMentions, "components" :=? components, "attachments" :=? attachments, "flags" :=? flags, "thread_name" :=? threadName)
  }
  ...
  def createFollowupMessage[MPR](
    ...
    Request.complexRestRequest(
      route = (Route.Empty / "webhooks" / Parameters[ApplicationId]("applicationId", applicationId) / Parameters[String]("interactionToken", interactionToken)).toRequest(Method.POST),
      params = body,
  ...
  def getFollowupMessage(
    ...
    Request.restRequest(
      route = (Route.Empty / "webhooks" / Parameters[ApplicationId]("applicationId", applicationId) / Parameters[String]("interactionToken", interactionToken) / "messages" / Parameters[MessageId]("messageId", messageId)).toRequest(Method.GET),
  ...
  object EditFollowupMessageBody extends DiscordObjectCompanion[EditFollowupMessageBody] with CreateMessageLikeMixin[EditFollowupMessageBody] {
    ...
    def make20(
        ...
        components: UndefOr[Seq[Component]] = UndefOrUndefined(Some("components")),
        attachments: UndefOr[Seq[ChannelRequests.MessageCreateEditAttachment]] = UndefOrUndefined(Some("attachments")),
    ): EditFollowupMessageBody = makeRawFromFields("content" :=? content, "embeds" :=? embeds, "allowed_mentions" :=? allowedMentions, "components" :=? components, "attachments" :=? attachments)
  }
  ...
  def editFollowupMessage[MPR](
    ...
    Request.complexRestRequest(
      route = (Route.Empty / "webhooks" / Parameters[ApplicationId]("applicationId", applicationId) / Parameters[String]("interactionToken", interactionToken) / "messages" / Parameters[MessageId]("messageId", messageId)).toRequest(Method.PATCH),
      params = body,
  ...
  def deleteFollowupMessage(
    ...
    Request.restRequest(
      route = (Route.Empty / "webhooks" / Parameters[ApplicationId]("applicationId", applicationId) / Parameters[String]("interactionToken", interactionToken) / "messages" / Parameters[MessageId]("messageId", messageId)).toRequest(Method.DELETE),
